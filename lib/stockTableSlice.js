 ...
 */
export const getStockInfoList = (num, location, stockInfoFromShipNoticeData, stockHandelRecs) =>
    dispatch => {
            ...
            .then(
                (data) => {
                    if (!_.isNaN(data) && !_.isEmpty(data) && _.size(data) > 0) {
                        let newData = dataProcess(preWork(data, stockHandelRecs), location, stockInfoFromShipNoticeData);
                        if (_.size(newData) > 0) {
 ...
 */
const dataProcess = (data, location, stockInfoFromShipNoticeData) => {
    ...
    let n = parseInt(stockInfoFromShipNoticeData.shipQty);
    let temp = 0;
    //let index = 0;
    let flag = true;
...
export const numExists = (num, data) => {
...
export const { saveStockData, doMaterialNum, getShipNoticeData, saveStockInfoFromShipNoticeData, saveLocationList, saveStockHandelRecords } = stockTableSlice.actions;
